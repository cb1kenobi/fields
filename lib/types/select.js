...
Select.prototype._validate = function _validate(value, cb) {
	...
	function checkValues(val) {
		var ival = self.ignoreCase ? val.toLowerCase() : val;
				...
				if (Array.isArray(self.complete)) {
					self.complete.forEach(function (key) {
						if (opt.hasOwnProperty(key)) {
							self._distinctValues[self.ignoreCase ? opt[key].toLowerCase() : opt[key]] = val;
						}
					});
				} else {
					self._distinctValues[self.ignoreCase ? val.toLowerCase() : val] = val;
				}
			});
