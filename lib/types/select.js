 ...
 *
 * @param {Function} callback - A function to call the selected item.
 */
		...
		if (Array.isArray(options)) {
			options.forEach(function (opt, idx) {
				var num = (new Array(3 - ('' + counter).length)).join(' ') + counter + ')  ',
					val = typeof opt === 'string'
							? opt
							: (self.optionLabel ? opt[self.optionLabel] : opt.label) || (self.optionValue ? opt[self.optionValue] : opt.value);
				counter++;
				...
				if (self.formatters.option) {
					...
					self._prerenderedOptions.push(
						(self.numbered ? num : '') +
						self._format(val, 'option')
					);
				}
				self._values[typeof opt === 'string' ? opt.replace(/__(.+?)__/g, '$1') : (self.optionValue ? opt[self.optionValue] : opt.value)] = 1;
			});
