...
/**
 * Base class for a creating a field.
 *
 * @class
 */
function Field() {
...
Field.prototype.colors = true;

Field.prototype.complete = false;

Field.prototype.defaultLeft = '[';

Field.prototype.defaultRight = ']';

Field.prototype.fieldSeparator = '\n';

Field.prototype.formatters = {};

Field.prototype.mask = '*';

Field.prototype.promptValuesLeft = '(';

Field.prototype.promptValuesRight = ')';

Field.prototype.promptValuesSeparator = '|';

Field.prototype.repromptOnError = true;

Field.prototype.separator = ': ';

Field.prototype.style = {
    default: 'cyan',
    input: 'magenta',
    title: 'bold',
    promptLabel: 'bold',
...
Field.prototype._format = function _format(str, style) {
	str = String(str);

	if (!style && this[str] !== undefined) {
				...
				if (ss) {
					if (Array.isArray(ss)) {
						ss.forEach(function (x) {
							str = str.map(function (i) {
								return /^__.+__$/.test(i) ? i : colors[x](i);
							});
						});
					} else {
						str = str.map(function (i) {
							return /^__.+__$/.test(i) ? i : colors[ss](i);
						});
					}
				}
			} else {
				str = str.map(function (i) {
					return /^__.+__$/.test(i) ? i : (colors.hasOwnProperty(style) ? colors[style](i) : i);
				});
			}
//		} catch (ex) {}
	}
...
Field.prototype._print = function _print(str) {
	process.stdout.write(str || '');
};

Field.prototype._println = function _println(str) {
	process.stdout.write((str || '') + '\n');
};

Field.prototype._getByNumber = function _getByNumber(value) {
	return value;
};

/**
 * Default wrapper around the user-defined validate().
 *
 * @param {String} value - The value that was entered at the prompt.
 * @param {Function} callback() - A function to call after validation completes.
 *
 * @returns {Boolean|undefined} - Returns true or false if value passes validation or undefined if this validate() is async.
 */
 Field.prototype._validate = function _validate(value, callback) {
	if (typeof this.validate === 'function') {
		return this.validate(value, callback);
	}
	return true;
};
 ...
 *
 * @param {Function} callback - A function to call after prompting completes.
 */
 Field.prototype._get = function _get(callback) {
	var prompt = '',
		margin = 0,
		caret = 0,
	...
	process.stdin.on('keypress', function(c, key) {
		...
		switch (key && key.name || c) {
			...
			case 'return':
				...
				try {
					var rethrow = false,
						result = this._validate(value, function (err, value) {
			...
			case 'backspace':
				if (caret && history[historyIdx].length) {
					...
					process.stdout.write('\x07'); // beep beep!
				}
				break;
